...
### Indice
[Pré-requisito (JDK 1.7)](### Primeiro passo - Pré-requisito (JDK 1.7))
[Download do maven](### Segundo passo - Download do maven)
[Descompactação](### Terceiro passo - Descompactação)
[Configurando as variáveis de ambiente](### Quarto passo - Configurando as variáveis de ambiente)
[Testando a instalação do maven](### Testando a instalação do maven)
...
Veja um exemplo da página de [download do maven](https://maven.apache.org/download.cgi).
...
4. Selecione a variável de ambiente PATH na lista e clique em Editar.
	Editar variável de ambiente PATH

5. Clique no botão Novo e adicione o caminho da pasta bin do maven: C:\maven\bin.
	Adicionando a pasta bin do maven na variável de ambiente PATH

6. Verifique se existe a variável de ambiente JAVA_HOME.
	Lembra que falei da variável de ambiente JAVA_HOME? Precisamos saber se ela já existe no seu sistema ou se precisamos criar.
...
Se não existir, você precisa criar essa nova variável de ambiente, clicando em Novo... e informando o nome JAVA_HOME e o valor com o caminho para a pasta onde o seu JDK está instalado. Por exemplo:

- Exemplo de configuração do JAVA_HOME
